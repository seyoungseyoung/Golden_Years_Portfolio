...
const Candlestick = (props: any) => {
  ...
  if (index < 5) {
      console.log(`Candlestick[${index}] PROPS:`, { x, y, width, height });
  ...
  if (!payload || typeof payload.open !== 'number' || typeof payload.close !== 'number' || typeof payload.high !== 'number' || typeof payload.low !== 'number') {
    if (index < 5) console.warn(`Candlestick[${index}]: Invalid or incomplete OHLC data in payload.`, {payload});
    return null; 
  }
  
  if (!yAxis || typeof yAxis.scale !== 'function') {
    if (index < 5) console.warn(`Candlestick[${index}]: yAxis or yAxis.scale is not available.`, {yAxis});
    return null; 
  }

  if (width <= 0) {
    if (index < 5) console.warn(`Candlestick[${index}]: width is zero or negative (${width}). Cannot draw.`);
    return null;
  }
  ...
  if (![actualOpen, actualClose, actualHigh, actualLow].every(val => Number.isFinite(val))) {
    if (index < 5) console.warn(`Candlestick[${index}]: Non-finite OHLC values in payload.`, {payload});
    return null;
  }
  ...
  if (index < 5) {
    console.log(`Candlestick[${index}] SCALED:`, { yOpen, yClose, yHigh, yLow });
  }

  if (![yOpen, yClose, yHigh, yLow].every(val => Number.isFinite(val))) {
    if (index < 5) console.warn(`Candlestick[${index}]: Non-finite scaled Y values.`, { yOpen, yClose, yHigh, yLow });
    return null;
  }
  ...
  } else if (actualOpen === actualClose) {
    calculatedBodyHeight = 1;
  }
...
const CustomTooltip = ({ active, payload, label }: any) => {
  if (active && payload && payload.length) {
    ...
    const dataPoint = pricePayload?.payload || payload[0].payload as StockDataPoint & { event?: SignalEvent };
...
export function StockPriceChart({ chartData, signalEvents }: StockPriceChartProps) {
  if (!chartData || !Array.isArray(chartData) || chartData.length === 0) {
    console.warn("StockPriceChart: chartData is invalid or empty.");
  ...
  if (hasInvalidFirstDataPoint) {
    console.error("StockPriceChart: First data point has invalid format.", { firstDataPoint });
  ...
  ).flatMap(d => [d.high, d.low]) as number[];

  if (pricesForDomain.length === 0) {
    console.error("StockPriceChart: No valid price data (finite high/low values) found for domain calculation.");
  ...
  if (yMin === yMax) {
    const spread = Math.max(1, Math.abs(yMin * 0.10));
    yMin -= spread;
    yMax += spread;
  } else {
  ...
  if (!isFinite(yMin) || !isFinite(yMax) || yMin >= yMax) {
      ...
      const typicalPrice = chartData[0].close;
  ...
  const volumeData = chartData.filter(d => typeof d.volume === 'number' && isFinite(d.volume)).map(d => d.volume as number);
  ...
  if (numDataPoints > 200) {
    xAxisTickInterval = Math.floor(numDataPoints / 12);
  } else if (numDataPoints > 60) {
    xAxisTickInterval = Math.floor(numDataPoints / 8); 
  } else if (numDataPoints > 20) {
    ...
    <div style={{ width: '100%', height: 450 }}>
      <ResponsiveContainer width="100%" height="70%">
        <ComposedChart data={dataWithSignals} margin={{ top: 5, right: 30, left: 5, bottom: 5 }}> 
          ...
          <Customized component={(props: any) => <Candlestick {...props} yAxisId="left" />} />
